...
const SidebarNav = ({ expanded, onToggle, collapsed, onLogout }) => {
  ...
  return (
    <div className="admin-sidebar__inner">
      ...
      <OverlayScrollbarsComponent className="admin-sidebar__scroll" options={scrollbarOptions} defer>
            ...
            return (
              <div key={item.id} className={`admin-sidebar__group ${isOpen ? 'is-open' : ''}`}>
                ...
                >
                  ...
                  {!collapsed && (
                    // <span className={`admin-sidebar__chevron bi ${isOpen ? 'bi-chevron-down' : 'bi-chevron-right'}`} aria-hidden="true" />
...
const Sidebar = ({ isMobile, show = false, onHide, collapsed }) => {
  ...
  return (
    <aside className={`admin-sidebar shadow-sm ${collapsed ? 'is-collapsed' : ''}`} data-bs-theme="dark">
      {content}
    </aside>
  );
};
