...
import { useConfirmResetPasswordMutation, useResetPasswordRequestMutation } from "#services/auth-api";
import { axiosBaseQueryUtil } from "#services/axios-config";
import { overlayPreloader } from "#utils";

import "./auth.scss";
import { CountDown } from "./register-page";
...
const ResetPasswordPage = () => {
  ...
  const handleRequestSubmit = requestForm.handleSubmit((values) => {
    ...
    axiosBaseQueryUtil.message = {
      success: "Đã gửi liên kết đặt lại mật khẩu (hiệu lực trong 2 phút)",
    };
    axiosBaseQueryUtil.overlay = overlayPreloader;
    axiosBaseQueryUtil.flowMessages = [
      { delay: 0, message: "Đang kiểm tra email..." },
      { delay: 1400, message: "Gửi liên kết đặt lại mật khẩu..." },
    ];
    axiosBaseQueryUtil.callbackfn = (response, hasError) => {
      if (hasError) {
        toast.error(response?.message || "Không thể gửi yêu cầu. Vui lòng thử lại.");
      }
    };
  ...
  const handleResetSubmit = resetForm.handleSubmit((values) => {
    ...
    axiosBaseQueryUtil.overlay = overlayPreloader;
    axiosBaseQueryUtil.flowMessages = [
      { delay: 0, message: "Đang xác thực liên kết..." },
      { delay: 1200, message: "Đang cập nhật mật khẩu mới..." },
    ];
    axiosBaseQueryUtil.callbackfn = (response, hasError) => {
      if (hasError) {
        toast.error(response?.message || "Liên kết không hợp lệ hoặc đã hết hạn");
      }
    };
  ...
  // useEffect(() => {
  //   if (isConfirmSuccess) {
  //     const timeoutId = setTimeout(() => navigate("/login"), 2200);
